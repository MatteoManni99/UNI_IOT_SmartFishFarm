...
#include "sys/log.h"
#define LOG_MODULE "Actuator Quality"
...
#define ID_ACTUATOR 2
#define TYPE_ACTUATOR "water_filter"
...
void handler(coap_message_t *response){
    ...
    {   
	    ...
	    LOG_INFO("Registration completed\n");
            registered = true;
	    // Partiamo da una condizione di OFF, imposto il led su RED
	    leds_on(LEDS_RED);
        }
    }
...
PROCESS(coap_server, "Quality Actuator");
...
PROCESS_THREAD(coap_server, ev, data)
{
    ...
    coap_activate_resource(&res_water_filter, "res_water_filter");
	...
	// Button implementation - Switch the mode
	if(ev == button_hal_release_event)
        {
            if(registered)
            {
                LOG_INFO("Button Pressed - Switching mode\n");
                res_water_filter.trigger();
            }
        }
