...
// Set sensor id - in our case there will be only one temp sensor with the id equals to 1
#define SENSOR_ID 1
...
#define SUB_TOPIC "temp_sampling"
#define PUB_TOPIC "temp"
...
PROCESS(mqtt_client_process, "MQTT Temperature Sensing");
...
{
    variation = rand()%(6); //rand from 0 to 5
...
static void message_handler(const char *topic, const uint8_t *chunk)
{
    ...
    if(strcmp((char*)topic, "temp_sampling") == 0)
    {
	// Json parsing
	   ...
	   {
           	printf("New temperature sampling period: %d\n sec", new_period);
...
static void mqtt_state_machine()
{
	...
	case STATE_INIT:
	    ...
	    //creating the json payload
            sprintf(pub_buffer, "{\"sensor_id\": \"%d\",\"temperature\":\"%d\"}", SENSOR_ID, temperature);
            LOG_INFO("Invio: %s\n", pub_buffer);
